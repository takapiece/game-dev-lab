...
class SoundFX {
  ...
  // Pass Catch (Crisp two-handed ball catch)
  playCatch() {
    if (!this.ctx) return;
    this.resume();
    const now = this.ctx.currentTime;
  ...
  // Steal / Poke ball sound (Crisp hand slap)
  playSteal() {
    if (!this.ctx) return;
    this.resume();
    const now = this.ctx.currentTime;
  ...
  // 24s Shot Clock Violation Buzzer (Stadium Electronic Horn)
  playBuzzer() {
    if (!this.ctx) return;
    this.resume();
    const now = this.ctx.currentTime;

    // Discordant dual sawtooth buzzer
    [164.81, 220.0].forEach(freq => {
      const osc = this.ctx.createOscillator();
      const gain = this.ctx.createGain();
  ...
  // NBA Jam "ON FIRE!" Awakening Sound (Ascending Power Arpeggio)
  playOnFire() {
    if (!this.ctx) return;
    this.resume();
    const now = this.ctx.currentTime;
    ...
    chord.forEach((freq, idx) => {
      ...
      const filter = this.ctx.createBiquadFilter();
      filter.type = 'lowpass';
      filter.frequency.setValueAtTime(800 + idx * 400, now + idx * 0.07);
      filter.frequency.exponentialRampToValueAtTime(3200, now + idx * 0.07 + 0.35);
